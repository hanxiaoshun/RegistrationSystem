...
def all_student_base_info_list(student_infos):
    ...
    try:
        ...
        day_files_path = file_root + 'all_student_base_info_list' + "/files/" + day_string
...
def create_data_user_(student_infos, file_day_files_path):
    ...
    data = pd.read_excel(document_root + "/all_info.xlsx",
                         sheet_name='按姓名统计')
    # system_type = platform.system()
    # if 'indows' in system_type:
    #     data = pd.read_excel("D:/PycharmProjects/lelingzdy/baoming/webapp/utils/all_info.xlsx",
    ...
    #
    # else:
    #     data = pd.read_excel("/opt/python3_space/lelingzdy/baoming/webapp/utils/all_info.xlsx",
    #                          sheet_name='按姓名统计')
    if len(student_infos) > 0:
        tmp_num = 0
        data.loc[0] = ['',
