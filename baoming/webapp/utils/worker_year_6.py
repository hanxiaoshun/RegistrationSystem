...
def worker_year_6(students, kwargs):
    """
    工作满6年（含）以上人员名单
    :param student:
    :return:
    ...
    try:
        ...
        work_unit = None
        skill = None
        if 'work_unit' in kwargs.keys() and 'declaration_of_occupation' in kwargs.keys():
            work_unit = kwargs['work_unit']
            skill = kwargs['declaration_of_occupation']
        if work_unit is None:
            return work_unit, skill
        
        document_root = os.path.join(BASE_DIR, 'document')
        tpl = DocxTemplate(document_root + '/worker_6_years_list.docx')
        tpl2 = DocxTemplate(document_root + '/worker_6_years_prov.docx')
        tmp_list = []
        tmp_num = 0
        if len(students) > 0:
        ...
        day_files_path = file_root + 'report_files/worker_years_6/' + student.declaration_of_occupation + "/" + str(
        ...
        if os.path.exists(file_day_files_path):
            file_manage.file_name = "工作满6年以上人员名单-" +  + work_unit + '-' + skill
        ...
        context = {
            'worker_unit':work_unit,
            'dec_of_occ':skill,
            'worker_number':len(students)
        }
        # 将附件 2 保存到文件系统
        ...
        if os.path.exists(file_day_files_path2):
            file_manage2.file_name = "证明（6年）-" + work_unit + '-' + skill
