...
def all_student_base_info_list():
    ...
    try:
        ...
        create_data_worker(student_infos_declaration_of_occupation, file_day_files_path)

        student_infos_real_name = StudentInfo.objects.filter(confirm_status=1).order_by('user_info')
        create_data_user(student_infos_real_name, file_day_files_path)

        student_infos_unit = StudentInfo.objects.filter(confirm_status=1).order_by('teacher_info')
        create_data_unit(student_infos_unit, file_day_files_path)
...
def create_data_worker(student_infos, file_day_files_path):
    ...
    if len(student_infos) > 0:
        ...
        DataFrame(data).to_excel(writer, sheet_name='按工种统计', index=False, header=0, na_rep='')
        writer.save()
        writer.close()
    else:
        pass
...
def create_data_user(student_infos, file_day_files_path):
    system_type = platform.system()
    if 'indows' in system_type:
        data = pd.read_excel("D:/PycharmProjects/lelingzdy/baoming/webapp/utils/all_info.xlsx",
                             sheet_name='按姓名统计')
...
def create_data_unit(student_infos, file_day_files_path):
    system_type = platform.system()
    if 'indows' in system_type:
        data = pd.read_excel("D:/PycharmProjects/lelingzdy/baoming/webapp/utils/all_info.xlsx",
                             sheet_name='按单位统计')
