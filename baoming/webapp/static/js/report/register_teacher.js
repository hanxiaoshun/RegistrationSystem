 ...
 */
//--------------------------------------------
function check_username_teacher() {
    /**
     * 检查用户名使用情况
     * @type {string}
     */
    ...
    let message_register = $("#message-register-teacher");

    if (value.length === 0) {
        username.css("border", "0.8px");
        username.css("solid", '#ff9dcb');
        $(".submit-teacher").attr("disabled", "disabled");
        ...
        $.ajax({
            ...
            type: 'POST',
            tradition: true,
            data: $("#register-form-teacher").serialize(),
            success: function (data) {
                ...
                if (data.status) {
                    ...
                    $(".submit-teacher").removeAttr('disabled');
                    let message_label = username.parent().next().children("span:first-child");
...
//--------------------------------------------
function check_nickname_teacher() {
    /**
     * 检查昵称是否有人注册
     * @type {string}
     */
    let url = '/report/check_nickname/';
    let nickname = $("#nickname-teacher");
...
function check_password_teacher() {
    /**
     * 检查密码的质量以及是否一致
     */
    let passwordReg = $("#passwordReg-teacher");
    let finalPWD = $("#finalPWD-teacher");
...
function CheckPassUsername_teacher(username) {//必须为字母加数字且长度不小于8位
    let str = username;
    let flag = true;
...
function CheckPassNickname_teacher(nickname) {
    /**
     *
     */
    let str = nickname;
...
function CheckPassWord_teacher(password) {//必须为字母加数字且长度不小于8位
    let str = password;
    let flag = true;
