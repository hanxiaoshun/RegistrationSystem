    ...
    try:
        if request.method == 'POST':
            ...
            if object_form.is_valid():
                ...
                if len(user_infos) > 0:
                    ...
                    if user_info_old.two_inch_photo:
                        if user_info_old.two_inch_photo.picture_path:
                            image_url = MEDIA_URL + str(user_info_old.two_inch_photo.picture_path)
                    else:
                        image_url = ''
                        # 学生附属信息
                ...
                print("primary_level::" + form_object_student_info.primary_level)
                ...
                if diploma_certificate_photos:
                    ...
                    term_worker_picture(school_term.school_term_name,
                                        ...
                                        username,
                                        "毕业证件照",
                                        form_object_student_info.diploma_certificate_photos.picture_path)
                if certificate_photos:
                    # 备份到对应的
                    term_worker_picture(school_term.school_term_name,
                                        form_object_student_info.declaration_of_occupation,
                                        username,
                                        "资格证件照",
                                        form_object_student_info.certificate_photos.picture_path)

                result['message'] = "学员信息添加成功！"
                result["level"] = log_level_add
                save_operation_log(request, inspect.stack()[0][3], form_object_student_info.__str__(True), result)
                ...
                if len(student_infos) > 0:
                    ...
                    return render_result(request, "page_main_controller/user/report_user_info_update.html",
                                         {'title_msg': title_msg, 'user_info': student_info.user_info,
                                          'student_id': student_id, 'image_url': image_url,
...
def report_student_info_list(request):
    ...
    username = request.session.get('username', None)  # 用户名
    register_user_info = RegisterUserInfo.objects.get(username=username)
    students = StudentInfo.objects.filter(user_info__register_user_info=register_user_info).order_by('-id')
    ...
    if SchoolTerm.objects.count() > 0:
        school_term = SchoolTerm.objects.last()
        return render_result(request, "page_main_controller/student/report_student_info_list.html",
                             {'title_msg': title_msg, "contacts": contacts, 'school_term': school_term,
                              'need_list': False})
    else:
        return render_result(request, "page_main_controller/student/report_student_info_list.html",
                             {'title_msg': title_msg, 'need_list': False})
    ...
    if request.method == 'GET':
        ...
        if int(student_id) > 0:
            ...
            if len(student_infos) == 0:
                ...
                return render_result(request, "page_main_controller/student/report_student_user_info_detail.html",
                                     {'title_msg': title_msg, 'student_info': student_info, 'image_url': image_url,
            ...
            if object_form.is_valid():
                ...
                if student_info_id:
                    ...
                    if len(student_infos) > 0:
                        ...
                        # 用户基础信息
                        form_object_student_info = form_to_obj(object_form.cleaned_data, student_info)
                        # 关联申报的级别信息
                        identification_level = object_form.cleaned_data.get('update_identification_level', None)
                        ...
                        diploma_certificate_photos = object_form.cleaned_data.get('diploma_certificate_photos_form',
                                                                                  None)
                        if diploma_certificate_photos:
                            diploma_certificate_photos_obj = Picture.objects.get(id=diploma_certificate_photos)
                            form_object_student_info.diploma_certificate_photos = diploma_certificate_photos_obj
                        ...
                        if diploma_certificate_photos:
                            # 备份到对应的
                            term_worker_picture(form_object_student_info.school_term.school_term_name,
                                                form_object_student_info.declaration_of_occupation,
                                                student_username,
                                                "毕业证件照",
                                                form_object_student_info.diploma_certificate_photos.picture_path)
                        ...
                        result['message'] = "学员信息添加成功！"
                        result["level"] = log_level_add
                        save_operation_log(request, inspect.stack()[0][3], form_object_student_info.__str__(True),
                                           result)
                        # user_info = UserInfo.objects.get(register_user_info=register_user_info.id)
                        # working_history_list = user_info.user_Info_working_history.all()
                        # print("user_Info_working_history::" + str(working_history_list))
                        ...
                        if form_object_student_info.user_info:
                            ...
                            if user_info.two_inch_photo:
                                ...
                                return render_result(request, "page_main_controller/user/report_user_info_update.html",
                                                     {'title_msg': title_msg, 'user_info': user_info,
                                                      'student_id': student_id, 'image_url': '',
                                                      'teacher_infos': teacher_infos,
                                                      'teacher_info_id': teacher_info_id})
                else:
                    print(type(object_form.errors), object_form.errors)  # errors类型是ErrorDict，里面是ul，li标签
            ...
            if len(student_infos) == 0:
                ...
                if student_info.school_term.status == "2":
                    ...
                    if editable:
                        ...
                        return render_result(request, "page_main_controller/student/report_student_info_update.html",
                                             {'title_msg': title_msg, 'student_info': student_info,
                                              'diploma_certificate_photos_url': diploma_certificate_photos_url,
                                              'certificate_photos_url': certificate_photos_url, 'not_report': False})
                    else:
                        # 状态修改修改权限也修改了
                        return render_result(request, "page_main_controller/message.html",
                                             {'title_msg': title_msg, 'message': message})
...
def report_student_submit(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if int(student_id) > 0:
                student_infos = StudentInfo.objects.filter(id=student_id).order_by('-id')
                if len(student_infos) == 1:
                    student_info = student_infos[0]
                    submit_status = student_info.submit_status
                    ...
                    if confirm_status == "2":
                        if review_status == "2":
                            if submit_status == "1":
                                ...
                                result['status'] = False
                                result['message'] = '所要申请的注销记录信息尚未提交或已经取消，请不要重复操作!'
                                result['data'] = ''
                        else:
                            result['status'] = False
                            result['message'] = '所要申请的记录已经由负责人审核通过，请联系负责人取消审核再修改!'
                            result['data'] = ''
                    else:
                        result['status'] = False
                        result['message'] = '您的报名信息已经通过最终审核，请联系负责人取消审核之后才能修改信息!'
                        result['data'] = ''
                else:
                    result['status'] = False
                    result['message'] = '所要提交的记录不存在!'
            ...
            if int(student_id) > 0:
                ...
                if len(student_infos) == 1:
                    ...
                    if submit_status == "1":
                        if review_status == "2":
                            if confirm_status == "2":
                                student_info.payment_amount = int(payment_amount)
                                student_info.unpaid_amount = int(unpaid_amount)
                                operation_object = student_info.id
                                student_info.save()
                                result['status'] = True
                                result['message'] = '缴费成功！已缴纳：' + str(payment_amount) + " 元。" + "未缴费：" + str(
                                    unpaid_amount) + " 元。"
                                result['data'] = json.dumps({}, ensure_ascii=False)
                            else:
                                result['status'] = False
